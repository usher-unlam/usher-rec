  ...
  with tf.Session(graph=detection_graph) as sess:
    ...
    if (i < FRAMES_OMITIDOS):
      ...
      for j in images_location:
        ...
        if coincide==True:
          locations_state=locations_state+"1"
          personas+=1
        else:
          locations_state=locations_state+"0"

      con = mysql.connector.connect(user="root",password="12345678",host="localhost",database="bbdd1")
      cursor=con.cursor()

      cursor.execute("INSERT INTO estado (estadoBancas) VALUES ('"+locations_state+"')")
      con.commit()
